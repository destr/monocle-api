...
hooks.after('Настройки > Построение карт РЛП > Получить настройки для построения карт РЛП', function(transaction) {
	var settings = JSON.parse(transaction.real.body);
...
hooks.after('Настройки > Параметры алгоритмов обработки метео данных > Получить текущее состояние', function(transaction) {
	var settings = JSON.parse(transaction.real.body)
...
hooks.after('Настройки экспорта > Настройка экспорта НАМС > Получить список настроек экспорта НАМС', function(transaction) {
	var settings = JSON.parse(transaction.real.body)
	if ((settings["items"][0].login != "login1") || (settings["items"][0].password != "password1")) {
...
hooks.after('Настройки экспорта > Настройка экспорта HDF5 > Получить список настроек экспорта HDF5', function(transaction) {
	var settings = JSON.parse(transaction.real.body)
	if ((settings["items"][0].login != "login1") || (settings["items"][0].password != "password1")) {
...
hooks.after('Настройки экспорта > Настройка экспорта MTP5 > Получить список настроек экспорта MTP5', function(transaction) {
	var settings = JSON.parse(transaction.real.body)
	if ((settings["items"][0].login != "login1") || (settings["items"][0].password != "password1")) {
