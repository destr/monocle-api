...
class JsonProtoGen:
    ...
    def _generate_enums_py(self, class_name, enums):

        self.classes[class_name] = "import enum\n\n" + self.classes[class_name]

        for k, e in enums.items():
            self.classes[class_name] += self._tab() + "class {0}(enum.Enum):\n".format(e.name)
            for k, v, t in e.items:
                self.classes[class_name] += self._tab(2) + "{0} = '{1}'\n".format(k, t)
    ...
    def _process_json_py(self, json_object, class_name):
        ...
        for key in json_object.keys():
            ...
            from_enum = [''] * 2
            if key in enums:
                to_enum[0] = class_name + "." + enums[key].name + "("
                to_enum[1] = ")"
    ...
    explicit {0}(const QJsonObject &obj) : is_null_(obj.isEmpty()) {{ assign(obj); }}\n""".format(class_name)
        ...
        if type(json_object) == list:
            is_root_array = True
            json_object = {"array": json_object}
            print("jsonprotogen.py:1:1: warning: Array as base type in {0} not supported! Only objects.".format(class_name),
                  file=sys.stderr)
